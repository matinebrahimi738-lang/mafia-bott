...
def assign_roles(players: list[str]) -> dict[str, str]:
    ...
    منطق: اول رول‌های کلیدی (غیر شهروند) رو با وزن مشخص پخش می‌کنه،
    بقیه‌ی نفرات شهروند می‌شن.
    """
    n = len(players)
    pool = eligible_roles(n)
    special = {k: v for k, v in pool.items() if k != "شهروند"}

    # حداکثر یک‌سوم بازیکن‌ها مافیا باشن (منطق متعادل رایج در بازی‌های جنگ/مافیا)
    max_mafia = max(1, n // 3)
    ...
    # اگه رول‌های خاص بیشتر از نفرات بود، رندوم کم می‌کنیم
    random.shuffle(role_list)
    role_list = role_list[: max(0, n - 1)]  # حداقل یک شهروند باقی بمونه
